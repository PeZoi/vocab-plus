...
import { formatRemainingTime, formatRelativeTime } from '@/utils/datetime';
...
export function DashboardRecentCards({ cards }: DashboardRecentCardsProps) {
  ...
  // Lấy các từ vựng đã đến hạn hoặc sắp đến hạn trong vòng < 1 ngày (< 24h)
  const urgentCards = useMemo(() => {
    if (!mountedTime) return [];

    const oneDayMs = 24 * 60 * 60 * 1000;
    const threshold = mountedTime + oneDayMs;

    const list = cards.filter((item) => {
      const uc = item.user_card;
      if (!uc?.due_at) return uc?.state === 'new';
      return new Date(uc.due_at).getTime() <= threshold;
    });

    // Sắp xếp: Thẻ đã đến hạn luôn ưu tiên lên đầu (due_at <= now),
    // tiếp theo là các thẻ sắp đến hạn (< 1 ngày).
    // Trong từng nhóm, sắp xếp thẻ có due_at sớm nhất lên trước.
    return list.sort((a, b) => {
      const dueA = new Date(a.user_card?.due_at || 0).getTime();
      const dueB = new Date(b.user_card?.due_at || 0).getTime();

      const isDueA = dueA <= mountedTime;
      const isDueB = dueB <= mountedTime;

      // 1. Thẻ đã đến hạn luôn ưu tiên lên đầu
      if (isDueA && !isDueB) return -1;
      if (!isDueA && isDueB) return 1;

      // 2. Cùng nhóm: thẻ có thời điểm đến hạn sớm hơn lên trước
      return dueA - dueB;
    });
  }, [cards, mountedTime]);

  // Đếm số lượng từ đã thực sự đến hạn ôn tập (due_at <= now)
  const dueCount = useMemo(() => {
    if (!mountedTime) return 0;
    return urgentCards.filter((item) => {
      const dueTime = item.user_card?.due_at
        ? new Date(item.user_card.due_at).getTime()
        : 0;
      return dueTime <= mountedTime;
    }).length;
  ...
  return (
    <div className="p-5 rounded-xl bg-surface/80 border border-border/70 flex flex-col justify-start gap-4 h-full">
        ...
        <div>
          ...
          <p className="text-[11px] text-text-secondary mt-1">
            {urgentCards.length > 0
              ? 'Các từ vựng đã đến hạn hoặc sắp đến hạn (< 1 ngày)'
        ...
        {dueCount > 0 ? (
          <Link href={ROUTES.APP.REVIEW} className="shrink-0">
            <Button size="sm" className="gap-1.5 text-xs font-semibold bg-brand hover:bg-brand-hover text-white shadow-sm shadow-brand/25 rounded-xl px-3.5 py-1.5">
              ...
              <span>Ôn tập ngay ({dueCount})</span>
            </Button>
          </Link>
        ) : (
          <Link href={ROUTES.APP.PRACTICE} className="shrink-0">
            <Button size="sm" variant="outline" className="gap-1.5 text-xs font-semibold rounded-xl px-3.5 py-1.5 text-text-secondary hover:text-text-primary">
      ...
      {cards.length === 0 ? (
        ...
        /* Vùng danh sách thẻ có cuộn mượt mà (Scrollable container - hiển thị khoảng 3 item) */
        <div className="max-h-[225px] overflow-y-auto custom-scrollbar pr-1 space-y-2.5">
          <motion.div
          ...
          >
            {urgentCards.map((item) => {
              const dueTime = item.user_card?.due_at
                ? new Date(item.user_card.due_at).getTime()
                : 0;
              const isOverdue = dueTime <= mountedTime;
                    ...
                    <span className="text-[10px] text-text-secondary/80 font-medium block">
                      {item.user_card?.due_at
                        ? isOverdue
                          ...
                          : formatRemainingTime(item.user_card.due_at, mountedTime)
