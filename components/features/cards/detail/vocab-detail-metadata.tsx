...
export function VocabDetailMetadata({ card }: VocabDetailMetadataProps) {
  return (
    <div className="rounded-2xl p-5 bg-surface/90 border border-border/70 shadow-sm space-y-4 text-xs">
      <div className="flex items-center gap-2 font-semibold text-text-secondary uppercase tracking-wider text-[11px]">
        <ShieldCheck className="w-4 h-4 text-brand" />
        <span>Thông tin quản lý</span>
      </div>

      <div className="space-y-2.5 divide-y divide-border/40 text-text-secondary">
        <div className="flex items-center justify-between pt-1">
          <span>Nguồn tạo thẻ:</span>
          <Badge variant="outline" className="text-[10px] uppercase text-text-secondary border-border">
            {card.source_type}
          </Badge>
        </div>
